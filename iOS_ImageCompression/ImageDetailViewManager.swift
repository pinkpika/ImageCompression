//
//  ImageDetailViewManager.swift
//  iOS_ImageCompression
//
//  Created by cm0620 on 2020/7/10.
...
/// 圖片詳細View管理者
class ImageDetailViewManager: NSObject{
    ...
    func zoomIn(imageView: UIImageView) {
        ...
        panStartLocation = CGPoint()
        imageStartMovingCenter = CGPoint()
    ...
    @objc private func clickMoreBtn(_ button: UIButton) {
        ...
        let saveImageAction = UIAlertAction(title: "儲存相片", style: .default){
            [weak self] _ in
    ...
    @objc func saveImage(_ image: UIImage, didFinishSavingWithError error: Error?, contextInfo: UnsafeRawPointer) {
        ...
        if error != nil {
            let alert = UIAlertController(title: "儲存失敗", message: nil, preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
            rootViewController.present(alert, animated: true, completion: nil)
        } else {
            let alert = UIAlertController(title: "已儲存", message: nil, preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "OK", style: .default, handler: nil))
            rootViewController.present(alert, animated: true, completion: nil)
        }
    }
...
extension ImageDetailViewManager: UIScrollViewDelegate{
